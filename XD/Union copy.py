...
class DirectMigrator:
    ...
    def create_onedrive_folder(self, folder_path: str) -> bool:
        ...
        for part in path_parts:
            ...
            folder_path_clean = current_path.strip('/')

            # Verificar si existe
            if folder_path_clean:
                check_url = f"https://graph.microsoft.com/v1.0/me/drive/root:/{folder_path_clean}"
            else:
                check_url = "https://graph.microsoft.com/v1.0/me/drive/root"
            ...
            if response.status_code == 404:
                ...
                if index > 0:
                    parent_path = '/'.join(path_parts[:index]).strip('/')
                    parent_url = f"https://graph.microsoft.com/v1.0/me/drive/root:/{parent_path}:/children"
                else:
                ...
                create_response = requests.post(parent_url, headers=headers, json=folder_data)
    ...
    def migrate_file_direct(self, file_info: Dict, remote_path: str) -> bool:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                final_remote_path = f"{self.onedrive_folder}/{remote_path}/{final_name}".replace('//', '/')
    ...
    def run_migration(self, skip_existing: bool = True):
        ...
        try:
            ...
            self.save_progress()
            print("\n Migración interrumpida. El progreso se ha guardado.")
...
def main():
    ...
    print(" Saltando archivos ya migrados automáticamente")
    print("="*50)
